...
export const createEarthMaterial4K = () => {
  return new THREE.MeshPhongMaterial({
    // 4k
    map: new THREE.TextureLoader().load('/images/4k/2_no_clouds_4k.jpg'),
...
export class Planet {
  ...
  async update () {
    ...
    const applyTerrain = (data = [], scale = 0.1) => {
      ...
      for (let i = 0, j = 0, l = vertices.length; i < l; i++, j += 3) {
        ...
        vector.setLength(data[i] * bumpScale + RADIUS)
        // console.log(data[i])
        vertices[j] = vector.x
    ...
    const applyTerrainPlane = (data = [], scale = 0.1) => {
      const geometry = this.mesh.geometry
      const vertices = geometry.attributes.position.array
      for (let i = 0, j = 0, l = vertices.length; i < l; i++, j += 3) {
